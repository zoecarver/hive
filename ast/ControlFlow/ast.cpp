...
Value* IfAST::codeGen() {
    ...
    Value* lastLineIf = ConstantInt::get(mContext, APInt(32, 0)); // so this can be used as turnary operator
    ...
    Value* lastLineElse = ConstantInt::get(mContext, APInt(32, 0)); // initialize to `0` so we have a value if the vector is empty
